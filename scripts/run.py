...
def create_invocations(settings):    
    ...
    qvbs_full = load_json(os.path.join(sys.path[0], "internal/qvbs-full.json"))
    qvbs_hard = load_json(os.path.join(sys.path[0], "internal/qvbs-hard.json"))
    premise = load_json(os.path.join(sys.path[0], "internal/premise.json"))
    benchmark_sets = OrderedDict()
    benchmark_sets["quickcheck"] = ["A single QVBS instance to quickly check if the installation was successful.", f"({len(quickcheck)} benchmarks)"]
    benchmark_sets["qvbs-full"] = ["All QVBS Benchmarks, including 'qvbs-hard' set", f"({len(qvbs_full)} benchmarks)"]
    benchmark_sets["qvbs-hard"] = ["Hard QVBS Benchmarks (Section 5.2)", f"({len(qvbs_hard)} benchmarks)"]
    benchmark_sets["premise"] = ["Runtime Monitoring  Benchmarks (Section 5.3)", f"({len(premise)} benchmarks)"]
    selected_benchmark_sets = input_selection("benchmark sets", benchmark_sets)
    selected_benchmark_ids = []
    selected_benchmarks = OrderedDict([(mt, []) for mt in ["dtmc", "ctmc", "mdp", "pta", "ma"]])
    if "qvbs-full" in selected_benchmark_sets:
        selected_benchmark_ids += qvbs_full
    else:
        if "quickcheck" in selected_benchmark_sets:
            selected_benchmark_ids += quickcheck
        if "qvbs-hard" in selected_benchmark_sets:
            selected_benchmark_ids += qvbs_hard
    if "premise" in selected_benchmark_sets:
        selected_benchmark_ids += premise
