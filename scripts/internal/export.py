...
def gather_exec_data(settings, logdirs):
    exec_data = OrderedDict() # Tool -> Config -> Benchmark -> Data
    benchmark_ids = []
    tools_configs = OrderedDict() # Tool X Config -> Num Supported
    for logdir_input in logdirs:
        logdir = os.path.expanduser(logdir_input)
        if not os.path.isdir(logdir):
            print("Error: Directory '{}' does not exist.".format(logdir))
    
        for res_json in [ load_json(os.path.join(logdir, f)) for f in os.listdir(logdir) if f.endswith(".json") and os.path.isfile(os.path.join(logdir, f)) ]:
            tool = res_json["tool"]
            ...
            exec_data[tool][config][benchmark] = res_json
            if not benchmark in benchmark_ids:
                benchmark_ids.append(benchmark)
            tools_configs.setdefault((tool,config), 0)
            if not is_not_supported(res_json):
                tools_configs[(tool,config)] += 1
    benchmark_ids.sort()
    return exec_data, benchmark_ids, tools_configs
...
def is_not_supported(result_json):
    return not result_json["supported"]
...
def generate_scatter_csv(settings, exec_data, benchmark_ids, tools_configs, property_filter=None):
    ...
    result = [ ["benchmark", "Type", "Orig", "Prop"] + ["{}.{}".format(t,c) for (t,c) in tools_configs] + ["best"] ]
            ...
            if benchmark_id in exec_data[tool][config]:
                ...
                if is_ignored(res_json):
                    ...
                    if is_memout(res_json):
                        ...
                        if not is_expected_error(res_json): print("Unexpected error for '{}.{}.{}'".format(tool, config, benchmark_id))
                        value = ERR_VALUE
                elif "timeout" in res_json and "wallclock-time" in res_json:
...
def generate_quantile_csv(settings, exec_data, benchmark_ids, tools_configs):
    ...
    result.append([1] + [MIN_VALUE] * (len(tools_configs) + len(runtimes_best_dict))) # prevents Latex error when there are no runtimes for a tool,config
    ...
    for (tool, config) in tools_configs:
        ...
        for benchmark_id in benchmark_ids:
            if benchmark_id in exec_data[tool][config]:
                res_json = exec_data[tool][config][benchmark_id]
                if "timeout" in res_json and res_json["timeout"] == False and ("result-correct" not in res_json or res_json["result-correct"] == True) and ("execution-error" not in res_json or res_json["execution-error"] == False) and "wallclock-time" in res_json:
...
def create_log_page(settings, result_json, output_dir):
    ...
    with open(os.path.join(output_dir, f_path), 'w') as f:
        ...
        write_line(f, indention, "<title>{}.{} - {} {} {}</title>".format(result_json["tool"], result_json["configuration-id"], b.get_model_short_name(), b.get_property_name(), b.get_parameter_values_string()))
...
def generate_table(settings, exec_data, benchmark_ids, tools_configs, output_dir):
    ...
    """)
        ...
        indention += 1
        for head in ["Model", "Type", "Original", "Parameters", "Property", "Type"] + ["{}.{}".format(t,c) for (t,c) in tools_configs]:
                ...
                if benchmark_id in exec_data[t][c]:
                    ...
                    if "timeout" in res_json and res_json["timeout"] == True:
                        ...
                        if is_memout(res_json):
                            ...
                            if not is_expected_error(res_json): print("Unexpected error for '{}.{}.{}' (found while creating table)".format(t, c, benchmark_id))
                    elif is_ignored(res_json):
                        res_str = "-"
                        link_attributes = " class='ignore'"
                    elif "timeout" in res_json and "execution-error" in res_json and "wallclock-time" in res_json:
                        res_str = "%.1f" % get_runtime(res_json)
                    else:
                        print("Unexpected execution result for '{}.{}.{}'".format(t, c, benchmark_id))
                    if res_str is not None:
                        logpage = create_log_page(settings, res_json, output_dir)
