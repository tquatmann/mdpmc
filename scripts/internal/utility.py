...
class Settings(object):
    ...
    def set_defaults(self):
        ...
        if not "storm-binary-dir" in self.json_data:
            self.json_data["storm-binary-dir"] = os.path.realpath(os.path.join(sys.path[0], "../tools/storm/build/bin/"))
            set_an_option = True
        if not "mcsta-binary-dir" in self.json_data:
            self.json_data["mcsta-binary-dir"] = os.path.realpath(os.path.join(sys.path[0], "../tools/Modest/"))
            set_an_option = True
